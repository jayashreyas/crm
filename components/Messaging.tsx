...
import { Send, Hash, Home, HandCoins, Info, Loader2 } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {threads.map(thread => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-5 py-4 rounded-[1.5rem] text-sm font-bold transition-all ${activeThreadId === thread.id ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' : 'text-slate-400 hover:bg-slate-200 hover:text-slate-700'
                }`}
            >
            ...
            return (
              <div key={msg.id} className={`flex items-end gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[70%] space-y-1`}>
                  ...
                  <div className={`px-6 py-4 rounded-[2rem] text-sm font-medium leading-relaxed shadow-sm ${isMe ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white text-slate-800 border rounded-bl-none'
                    }`}>
                    {msg.text}
                  </div>

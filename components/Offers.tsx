...
export const Offers: React.FC<OffersProps> = ({ offers, listings, users, currentUser, onUpdateStatus, onImport, onRefresh, onAddOffer }) => {
  ...
  const handleAISummary = async (e: React.MouseEvent, offer: Offer) => {
    ...
    try {
      if (db.consumeCredits(currentUser.agencyId, currentUser.id, 2)) {
        const listing = listings.find(l => l.id === offer.listingId);
        if (listing) {
          const summary = await AIService.summarizeOffer(offer, listing);
          db.updateOfferSummary(offer.id, summary);
          onRefresh();
  ...
  const handleSaveOfferDetails = (e: React.FormEvent) => {
    ...
    db.saveOffer(updatedOffer, currentUser.id);
    db.logActivity(currentUser.agencyId, currentUser.id, 'updated negotiation terms for', updatedOffer.buyerName);
    onRefresh();
              ...
              {offers.filter(o => o.status === stage).map(offer => {
                ...
                return (
                  <div 
                    ...
                    className={`bg-white p-6 rounded-[2rem] shadow-sm border transition-all group cursor-pointer relative ${
                      selectedOffer?.id === offer.id ? 'border-indigo-500 shadow-indigo-100 ring-2 ring-indigo-50' : 'border-transparent hover:border-indigo-300 hover:shadow-xl'
                    }`}
                  >
              ...
              <div className="grid grid-cols-3 gap-2">
                {stages.map((stage) => (
                  <button
                    ...
                    className={`px-3 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border-2 transition-all ${
                ...
                {['Inspection', 'Appraisal', 'Financing', 'Sale of Home', 'Title Search'].map(c => (
                  <button 
                    ...
                    className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border-2 transition-all ${
                      (editForm.contingencies || []).includes(c) 
